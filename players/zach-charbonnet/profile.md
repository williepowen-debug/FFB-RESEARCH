---
schema_version: 1
record_id: pp-all-zach-charbonnet-001
record_type: player_profile
title: "Zach Charbonnet"
team_ids: ["SEA"]
player_ids: ["local-player-zach-charbonnet-2023"]
season: null
week: null
status: active
time_horizon: durable
valid_as_of: 2026-09-09
last_verified: 2026-09-09
confidence: medium
source_ids: ["local-source-wikipedia-nfl-stats", "local-source-nfl-com", "local-source-seattle-seahawks", "local-writer-john-boyle", "local-source-espn", "local-source-pro-football-reference"]
supersedes: []
---

# Zach Charbonnet

- Position: RB
- Current team: Seattle Seahawks (reserve/physically unable to perform)
- Season: durable profile (verified 2026-09-09)
- Last verified: 2026-09-09
- Status: active

## Durable player profile

### Traits and role fit

Charbonnet is a between-the-tackles, contact-balance runner and a capable receiver
whose NFL value has been as the second back and goal-line finisher behind Kenneth
Walker III. His college production was elite on volume (1,137 and 1,359 rushing yards
in his two UCLA seasons at 5.6 and 7.0 yards per carry, with 37 receptions in 2022),
but his NFL efficiency has been ordinary: 4.3, 4.2, and 4.0 yards per carry across
three seasons. What he has done is score: 12 rushing touchdowns on 184 carries in 2025
with zero starts, the clearest goal-line-back profile on the roster.

### Historical usage and production

NFL:

| Season | Team | GP/GS | Rush att | Rush yds | Y/A | Rush TD | Tgt | Rec | Rec yds | Rec TD | Fum |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 2023 | SEA | 16/2 | 108 | 462 | 4.3 | 1 | n/v | 33 | 209 | 0 | 0 |
| 2024 | SEA | 17/11 | 135 | 569 | 4.2 | 8 | n/v | 42 | 340 | 1 | 0 |
| 2025 | SEA | 16/0 | 184 | 730 | 4.0 | 12 | n/v | 20 | 144 | 0 | 0 |
| Career | SEA | 49/13 | 427 | 1,761 | 4.1 | 21 | — | 95 | 693 | 1 | 0 |

College:

| Season | School | GP | Rush att | Rush yds | Y/A | Rush TD | Rec | Rec yds |
|---|---|---|---|---|---|---|---|---|
| 2019 | Michigan | 13 | 149 | 726 | 4.9 | 11 | 8 | 30 |
| 2020 | Michigan | 5 | 19 | 124 | 6.5 | 1 | 6 | 41 |
| 2021 | UCLA | 12 | 202 | 1,137 | 5.6 | 13 | 24 | 197 |
| 2022 | UCLA | 10 | 195 | 1,359 | 7.0 | 14 | 37 | 321 |

Sources: Wikipedia and NFL.com season tables, verified 2026-09-09. Targets are marked
n/v (not verified) because Pro Football Reference returned HTTP 403 on 2026-09-09.

Three patterns matter.

**Carries rose every year, efficiency fell every year.** 108, 135, 184 attempts at 4.3,
4.2, 4.0 yards per carry. He was never the explosive back; Walker was.

**Touchdowns are the asset.** 8 and then 12 rushing touchdowns in the last two seasons
on a backup's carry count. Twelve scores on 184 carries (6.5 percent) is a goal-line
role, and it was earned with zero starts in 2025.

**Receiving usage collapsed in 2025.** From 42 receptions in 2024 to 20 in 2025 as
Seattle's offense funneled targets to Smith-Njigba and Barner. His PPR case weakened
even as his touchdown case strengthened.

Durability before the injury: 49 of 51 possible regular-season games, no fumbles in
three seasons.

### Strengths, limitations, and relevant context

Strengths: proven goal-line finisher; zero career fumbles; three-down capability shown
in 2024 (42 catches); familiarity with the Shanahan-tree structure the new
coordinator keeps.

Limitations: a torn ACL suffered 2026-01-19 in the divisional-round win over San
Francisco, with surgery in February 2026; sub-4.3 yards per carry every NFL season; a
first-round rookie drafted into his absence; a contract year (2023 second-round rookie
deal runs through 2026).

Context: Seattle placed him on the active/PUP list 2026-07-22 and moved him to
reserve/PUP at the 2026-08-30 cutdown, which requires him to miss at least the first
four games; under the current rule he can return to practice as early as Week 3 and
is first eligible to play in Week 5 against San Francisco. Macdonald said in July that
he was doing a great job in recovery, with no timetable set.

## Current situation

### Team and depth-chart role

Reserve/PUP; not eligible to play until Week 5 (2026-10-11 or later, depending on
schedule verification). While he is out the backfield is Jadarian Price (listed first
on the 2026-09-09 depth chart), Emanuel Wilson, and George Holani. The official
framing is that Price will not carry the load alone; ESPN's Henderson wrote that the
role will shift once Charbonnet returns. Nothing about his post-return role has been
stated by the team. See
[post-Walker backfield](../../teams/NFC/West/Seattle-Seahawks/2026/offense/post-walker-backfield.md)
and hypothesis `sea-off-rb-001` in
[offense hypotheses](../../teams/NFC/West/Seattle-Seahawks/2026/offense/hypotheses.csv).

### Coaching/scheme fit

Fleury's offense is a continuity build from the Kubiak system in which Charbonnet
scored 12 touchdowns, so scheme fit is not the question; timing and the rookie's
performance are. If Price establishes the lead role in Weeks 1-4, Charbonnet's most
likely re-entry point is the goal-line and short-yardage role he already held. See
[Fleury continuity finding](../../teams/NFC/West/Seattle-Seahawks/2026/offense/fleury-continuity-offense.md).

### Health and availability

Out through at least Week 4 on reserve/PUP (torn ACL 2026-01-19, surgery February
2026). Return to practice is possible from Week 3; the team has published no timetable.
An ACL return roughly eight to nine months after surgery is on the aggressive side for
a running back, so the first eligible week should not be assumed to be the actual
return week.

## Fantasy assessment

- Applicable format: dynasty and deep redraft benches now; a touchdown-dependent
  flex in standard scoring once active. Best value in leagues with injured-reserve
  slots.
- Range of outcomes: from a Week 5-8 return into a goal-line committee role (6-10
  carries, high touchdown rate, few catches) to a lead role by November if Price
  struggles or is hurt; to a season-long depth role if Price seizes the job and the
  knee delays him.
- Key upside condition: Price failing to secure goal-line or passing-down work in
  Weeks 1-4, leaving both roles open at Charbonnet's return; or a Price injury.
- Key downside condition: Price producing as a three-down back before Week 5 and
  Charbonnet returning slowly, making him the third option in a run-heavy but
  concentrated backfield.
- Confidence: high that he misses Weeks 1-4; low on the return date; low on the
  post-return share.

## Team history

- 2023-present: Seattle Seahawks. Selected in the second round (52nd overall) of the
  2023 NFL Draft out of UCLA (transfer from Michigan, 2019-2020). Placed on active/PUP
  2026-07-22 and reserve/PUP 2026-08-30 after a torn ACL suffered 2026-01-19.

## What would change this assessment

- The date he returns to practice (eligible from Week 3) and the date he is activated;
  activation in Week 5 signals a fast recovery, activation after Week 8 signals a
  slower one.
- Price's carry, goal-line, and passing-down shares through Week 4; each role Price
  does not secure is a role Charbonnet can reclaim.
- Any team statement about a post-return hierarchy or a pitch count.
- A knee setback in the return-to-practice window.
- Any Seattle transaction adding a veteran back, which would signal doubt about the
  timeline.

## Sources

- Wikipedia — [Zach Charbonnet](https://en.wikipedia.org/wiki/Zach_Charbonnet) — NFL and college statistics, draft, ACL date, PUP status, verified 2026-09-09.
- NFL.com — [Zach Charbonnet stats](https://www.nfl.com/players/zach-charbonnet/stats/) — season lines, verified 2026-09-09.
- Pro Football Reference — [Zach Charbonnet](https://www.pro-football-reference.com/players/C/CharZa00.htm) — attempted 2026-09-09; returned HTTP 403, not used.
- John Boyle, Seahawks.com — [Seahawks RB Zach Charbonnet Placed On PUP List](https://www.seahawks.com/news/seahawks-rb-zach-charbonnet-placed-on-pup-list) — published 2026-07-22, verified 2026-09-09.
- John Boyle, Seahawks.com — [Seahawks Make Roster Moves, Including Trade, To Establish Initial 53-Man Roster](https://www.seahawks.com/news/seahawks-make-roster-moves-including-trade-to-establish-initial-53-man-roster) — published 2026-08-30; reserve/PUP, at least four games; verified via team finding 2026-09-09.
- ESPN — [Seahawks' Zach Charbonnet still on PUP list, out first 4 games](https://www.espn.com/nfl/story/_/id/49778534/seahawks-zach-charbonnet-remains-pup-list-miss-least-first-four-games) — August 2026; Week 3 practice eligibility and Week 5 return window; verified 2026-09-09 via search summary.
- Brady Henderson, ESPN — [Are the Seahawks ready for rookie Jadarian Price to lead run game?](https://www.espn.com/nfl/story/_/id/49778349/seahawks-rookie-rb-jadarian-price-ready-lead-run-game) — published 2026-08-31; February surgery and role-shift note; verified 2026-09-09.
- Repository — [SEA synthesis 2026-09-09](../../intelligence/2026/syntheses/2026-09-09/SEA.md).
