---
schema_version: 1
record_id: pp-all-george-holani-001
record_type: player_profile
title: "George Holani"
team_ids: ["SEA"]
player_ids: ["local-player-george-holani-2024"]
season: null
week: null
status: active
time_horizon: durable
valid_as_of: 2026-09-10
last_verified: 2026-09-10
confidence: medium
source_ids: ["local-source-nfl-gamebooks", "local-source-wikipedia-nfl-stats", "local-source-nfl-com", "local-source-seattle-seahawks", "local-writer-john-boyle", "local-writer-brady-henderson", "local-source-espn", "local-source-pro-football-reference"]
supersedes: []
---

# George Holani

- Position: RB
- Current team: Seattle Seahawks
- Season: durable profile (verified 2026-09-09)
- Last verified: 2026-09-10
- Status: active

## Durable player profile

### Traits and role fit

Holani is a former Boise State lead back whose NFL work has primarily been depth and
special teams. Seattle's official biography lists 3,596 college rushing yards and
1,000-yard seasons in 2019 and 2022. That corrects the earlier 3,569-yard figure and
claim of two 1,100-yard seasons. His NFL sample is 25 carries for 83 yards through 2025;
college production alone does not prove an NFL three-down role.

### Historical usage and production

NFL:

| Season | Team | GP/GS | Rush att | Rush yds | Y/A | Rush TD | Tgt | Rec | Rec yds | Rec TD |
|---|---|---|---|---|---|---|---|---|---|---|
| 2024 | SEA | 5/0 | 3 | 10 | 3.3 | 0 | n/v | 1 | -1 | 0 |
| 2025 | SEA | 11/0 | 22 | 73 | 3.3 | 1 | n/v | 2 | 15 | 0 |
| Career | SEA | 16/0 | 25 | 83 | 3.3 | 1 | — | 3 | 14 | 0 |

Correction checked September 9: the official Seahawks biography resolves the earlier
23-versus-22 attempt conflict in favor of 22 for 2025 (3 + 22 = 25 career carries).
The earlier secondary-source college table was removed because its totals conflicted
with the club biography. The club verifies 1,014 rushing yards and 26 catches for 206
yards in 2019, and 1,157 rushing yards in 2022; other college season rows remain unaudited.

Pattern: a productive, receiving-capable college workhorse whose NFL usage has been
almost entirely situational. There is no NFL evidence of efficiency on volume.

### Strengths, limitations, and relevant context

Strengths: a receiving and pass-protection reputation that gives him a defined
passing-down role behind a rookie; experience in Seattle’s offense; exclusive-rights tender keeps him cheap and retained.

Limitations: 3.3 yards per carry on 25 NFL attempts; zero NFL starts; a hamstring
injury that ended his 2025 regular season on 2025-11-29 (injured after the 2025-11-23
Tennessee game); a first-round rookie ahead of him and a veteran (Emanuel Wilson)
beside him.

Context: retained on an exclusive-rights tender (tendered 2026-03-02, signed
2026-03-16) for 2026 only.

## Current situation

### Team and depth-chart role

Third on the official depth chart as of 2026-09-09 (Price, Wilson, Holani), down from
first on the 2026-08-17 chart — an ordering change, not a usage measurement. The
official 53-man analysis (2026-08-30) said Price and Holani expect to split the
workload early; the Week 1 preview (2026-09-07) named him among the backs behind Price
so that Price does not carry the load alone; ESPN's Brady Henderson (2026-08-31)
described his role as heavy involvement on passing downs and third down. He carries no
Week 1 designation. Zach Charbonnet is on reserve/PUP for at least four games. See
[post-Walker backfield](../../teams/NFC/West/Seattle-Seahawks/2026/offense/post-walker-backfield.md)
and the [SEA synthesis 2026-09-09](../../intelligence/2026/syntheses/2026-09-09/SEA.md).

### Coaching/scheme fit

Fleury's Shanahan-tree structure uses the back heavily in the screen and check-down
game, which is the role Holani is described as filling. The trade-off is that the same
structure concentrates carries in the lead back once one is established. See
[Fleury continuity finding](../../teams/NFC/West/Seattle-Seahawks/2026/offense/fleury-continuity-offense.md).

### Health and availability

Available, no designation. 2025: hamstring injury, injured reserve 2025-11-29,
activated for the NFC Championship Game. His official transaction history also records a 2024 ankle absence on practice-squad injured reserve.

## Week 1 2026 — measured usage

*New England at Seattle, 2026-09-09. Seattle won 13-10.*

**23 of 50 offensive snaps (46%), 8 of 22 carries for 29 yards (3.6 avg, long 9), 1 target, 0 receptions.**

Holani matched rookie first-rounder Jadarian Price almost exactly in playing time — 23 snaps to 24 —
while trailing him in efficiency. The significance is not Holani's own line but what it establishes
about the backfield: **there is no lead back in Seattle after one game.**

Source: official gamebook for `2026-W01-NE-SEA`, retrieved 2026-09-10. The participation table is labelled "Unofficial" by the document; no official participation report exists for this game.
Full measured rows: [Week 1 usage sheet](../../weekly/2026/week-01/usage-input.csv). Matchup record: [NE at SEA](../../weekly/2026/week-01/games/NE-at-SEA.md).

## Fantasy assessment

- Applicable format: full PPR (passing-down role) and deep or dynasty leagues as a
  handcuff; not a standard-scoring starter.
- Range of outcomes: from a pure passing-down back at 3-6 touches a game to a 10-14
  touch committee partner if Seattle protects Price on third downs and near the goal
  line. His single-game ceiling is highest in weeks Seattle trails.
- Key upside condition: an injury to Price, or Price failing in pass protection, which
  could expand Holani’s role; Emanuel Wilson has more NFL rushing experience.
- Key downside condition: Price handling all three downs and Wilson taking short
  yardage, leaving Holani a special-teams back; Charbonnet's return in Week 5 or later
  pushes him further down.
- Confidence: medium on a defined passing-down role for Weeks 1-4; low on touches
  beyond that; low on any standalone value after Charbonnet returns.

## Team history

- 2024-present: Seattle Seahawks. Signed as an undrafted free agent out of Boise State
  on 2024-05-03; waived 2024-08-27 and re-signed to the practice squad 2024-08-28;
  signed to the active roster 2024-12-26. Placed on injured reserve 2025-11-29 and
  activated for the 2025 NFC Championship Game. Tendered as an exclusive-rights free
  agent 2026-03-02 and signed 2026-03-16.

## What would change this assessment

- Week 1 route and third-down snaps: if he runs the majority of backfield routes he
  has a PPR role; if Price or Wilson does, he is a depth back.
- Any Price absence or a visible third-down substitution pattern against Price.
- Wilson taking the short-yardage and two-minute work, which would remove Holani's
  second path to touches.
- Charbonnet's announced return-to-practice and activation dates; eligibility alone is not an activation forecast.
- A recurrence of the 2025 hamstring injury.

### NFL statistical audit — September 9, 2026

Available regular-season count fields were cross-checked against the [official club career table](https://www.seahawks.com/team/players-roster/george-holani/).
The [audit snapshot](../../weekly/2026/week-01/research/ne-sea/audit/official-career-counts.csv)
retains the source counts; [verification limits](../../weekly/2026/week-01/research/ne-sea/audit/README.md)
separate targets, return statistics, college history and source-average discrepancies.
Blank source fields remain unverified; this audit does not independently confirm every
older target, return or postseason statistic. Derived averages use their stated
numerator/denominator. This check does not reverify
contracts, every scouting claim or current game status.

## Sources

- [Official club biography and career statistics](https://www.seahawks.com/team/players-roster/george-holani/) — dynamic; NFL count columns checked 2026-09-09.

- Seahawks — [George Holani biography and career table](https://www.seahawks.com/team/players-roster/george-holani/) — dynamic; checked 2026-09-09; controls the corrected NFL attempts and college totals.

- Wikipedia — [George Holani](https://en.wikipedia.org/wiki/George_Holani) — NFL statistics, college totals and season prose, transactions, injured-reserve dates, verified 2026-09-09.
- NFL.com — [George Holani stats](https://www.nfl.com/players/george-holani/stats/) — season rushing and receiving lines, verified 2026-09-09.
- Pro Football Reference — [George Holani](https://www.pro-football-reference.com/players/H/HolaGe00.htm) — attempted 2026-09-09; returned HTTP 403, not used.
- ESPN — [Seahawks activate George Holani, Elijah Arroyo for NFC title game](https://www.espn.com/nfl/story/_/id/47707108/seahawks-activate-george-holani-elijah-arroyo-nfc-title-game) — January 2026 activation and injury date, verified 2026-09-09 via search summary.
- Seahawks.com — [Seahawks Activate RB George Holani & TE Elijah Arroyo From IR](https://www.seahawks.com/news/2025-seahawks-activate-rb-george-holani-te-elijah-arroyo-from-ir) — January 2026, verified 2026-09-09 via search summary.
- Brady Henderson, ESPN — [Are the Seahawks ready for rookie Jadarian Price to lead run game?](https://www.espn.com/nfl/story/_/id/49778349/seahawks-rookie-rb-jadarian-price-ready-lead-run-game) — published 2026-08-31; Holani passing-down role, verified 2026-09-09.
- John Boyle, Seahawks.com — [What To Watch In The Seahawks' Season Opener Against The Patriots](https://www.seahawks.com/news/what-to-watch-in-the-seahawks-season-opener-against-the-patriots) — published 2026-09-07, verified via synthesis 2026-09-09.
- John Boyle, Seahawks.com — [A Position-By-Position Look At The Seahawks' Initial 2026 53-Man Roster](https://www.seahawks.com/news/a-position-by-position-look-at-the-seahawks-initial-2026-53-man-roster) — published 2026-08-30, verified via team finding 2026-09-09.
- Repository — [Week 1 NE at SEA](../../weekly/2026/week-01/games/NE-at-SEA.md).
