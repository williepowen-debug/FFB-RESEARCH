---
schema_version: 1
record_id: pp-all-corey-kiner-001
record_type: player_profile
title: "Corey Kiner"
team_ids: ["NE"]
player_ids: ["local-player-corey-kiner-2025"]
season: null
week: null
status: active
time_horizon: durable
valid_as_of: 2026-09-09
last_verified: 2026-09-09
confidence: medium
source_ids: ["local-source-new-england-patriots"]
supersedes: []
---

# Corey Kiner

- Position: RB
- Current team: NE
- Last verified: 2026-09-09
- Scope: durable history and role fit; current game status lives in the [weekly matchup](../../weekly/2026/week-01/games/NE-at-SEA.md).

## Durable player profile

### Traits and role fit

A compact runner with a very small NFL sample. New England’s acquisition announcement lists him at 5-foot-9 and 209 pounds. His historical production establishes that he has played NFL snaps, but offers little basis for ranking his protection or receiving ability.

### Historical usage and production

| Season | Team | Carries | Rush yards | Receptions | Receiving yards |
|---|---|---|---|---|---|
| 2025 | ARI | 12 | 58 | 2 | 15 |

The Patriots’ trade announcement also records one kickoff return for 22 yards. Avoid extrapolating an efficiency or three-down role from twelve carries.

### Strengths, limitations, and relevant context

He moved through multiple organizations in his rookie season. Familiarity with New England’s protections and assignments is a separate question from running talent.

## Current situation

### Team and depth-chart role

Acquired from Arizona for a 2028 seventh-round selection. The dated team article is August 28, 2026; the transaction index lists August 29. Preserve that publication/index discrepancy. The weekly matchup contains his injury-forced opportunity behind Stevenson.

### Coaching/scheme fit

ARCH inference: he can provide relief carries, but meaningful passing-down responsibility would require evidence of staff trust and execution. Neither a trade nor active status proves that responsibility.

### Health and availability

Use the weekly matchup’s official eligibility check. No multi-season durability conclusion is possible from this sample.

## Fantasy assessment

- Applicable format: general; role-dependent value in PPR, best ball and deeper leagues.
- Range of outcomes: complementary involvement through a meaningful weekly role; no numerical projection before current routes and touches are established.
- Key upside condition: Standalone series plus passing assignments or short-yardage work, rather than only relief carries.
- Key downside condition: Stevenson retains most important snaps and the third back handles other limited work.
- Confidence: medium on the historical role description; low on precise current opportunity.

## Team history

2025: entered with San Francisco as an undrafted Cincinnati player; later Minnesota practice squad, then Arizona active roster. Dressed but did not play in one Minnesota game. 2026: traded to New England.

## What would change this assessment

Does Kiner receive standalone series or passing and short-yardage assignments? Compare those opportunities with Stevenson and the other active backs; relief carries alone establish little.
Separate injury-forced opportunity and score state from the healthy-roster baseline.

### NFL statistical audit — September 9, 2026

Available regular-season count fields were cross-checked against the [official club career table](https://www.patriots.com/team/players-roster/corey-kiner/).
The [audit snapshot](../../weekly/2026/week-01/research/ne-sea/audit/official-career-counts.csv)
retains the source counts; [verification limits](../../weekly/2026/week-01/research/ne-sea/audit/README.md)
separate targets, return statistics, college history and source-average discrepancies.
Blank source fields remain unverified; this audit does not independently confirm every
older target, return or postseason statistic. Derived averages use their stated
numerator/denominator. This check does not reverify
contracts, every scouting claim or current game status.

## Sources

- [Official club biography and career statistics](https://www.patriots.com/team/players-roster/corey-kiner/) — dynamic; NFL count columns checked 2026-09-09.

- [Patriots — Corey Kiner acquisition announcement](https://www.patriots.com/news/patriots-acquire-rb-corey-kiner-in-a-trade-with-arizona-release-rb-hassan-haskins) — published 2026-08-28; checked 2026-09-09.
- [Patriots transaction index](https://www.patriots.com/team/transactions/2026) — dynamic; checked 2026-09-09.
