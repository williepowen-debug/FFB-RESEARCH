---
schema_version: 1
record_id: pp-all-cooper-kupp-001
record_type: player_profile
title: "Cooper Kupp"
team_ids: ["SEA"]
player_ids: ["local-player-cooper-kupp-2017"]
season: null
week: null
status: active
time_horizon: durable
valid_as_of: 2026-09-09
last_verified: 2026-09-09
confidence: medium
source_ids: ["local-source-seattle-seahawks"]
supersedes: []
---

# Cooper Kupp

- Position: WR
- Current team: SEA
- Last verified: 2026-09-09
- Scope: durable history and role fit; current game status lives in the [weekly matchup](../../weekly/2026/week-01/games/NE-at-SEA.md).

## Durable player profile

### Traits and role fit

A route-detail and coverage-recognition receiver whose role has evolved from offensive centerpiece to veteran complement. John Boyle’s August 26 feature documents his participation in quarterback meetings and practice performance; leadership praise is not a target-share measurement.

### Historical usage and production

| Season | Team | Receptions | Receiving yards | TD |
|---|---|---|---|---|
| 2021 | LAR | 145 | 1,947 | 16 |
| 2023 | LAR | 59 | 737 | 5 |
| 2024 | LAR | 67 | 710 | 6 |
| 2025 | SEA | 47 | 593 | 2 |

Official club career table. His 2025 volume was far below the 2021 peak despite 12.6 yards per catch. Production per reception does not establish that his earlier target-earning role will return.

### Strengths, limitations, and relevant context

The club biography records prior knee, ankle and hamstring injured-reserve stints. His experience reading coverage matters to the offense, but age, availability and competing receivers make a return to peak volume an unsupported baseline.

## Current situation

### Team and depth-chart role

Part of the established Seattle receiver group with Smith-Njigba and Shaheed. Boyle’s August feature names that trio and reports a strong camp. No 2026 regular-season route percentage is asserted.

### Coaching/scheme fit

ARCH inference: Kupp is an alternative when defenses devote resources to Smith-Njigba and can help maintain timing in Fleury’s offense. His role means additional opportunity for Shaheed or Barner cannot be allocated without also accounting for Kupp.

### Health and availability

Use the weekly matchup for current eligibility. The August feature says his joint-practice workload was managed and he did not appear in preseason games; neither establishes a new injury or a lost role.

## Fantasy assessment

- Applicable format: general; role-dependent value in PPR, best ball and deeper leagues.
- Range of outcomes: complementary involvement through a meaningful weekly role; no numerical projection before current routes and touches are established.
- Key upside condition: Stable routes, third-down opportunities and favorable matchups when help follows Smith-Njigba.
- Key downside condition: A low-volume offense and target concentration around Smith-Njigba leave few opportunities; reduced participation or injury adds further downside.
- Confidence: medium on the historical role description; low on precise current opportunity.

## Team history

2017–2024: Los Angeles Rams, drafted in Round 3 (69th). 2025 onward: Seattle, signed March 18, 2025 after his Rams release.

## What would change this assessment

Does Kupp retain regular routes and third-down opportunities when defenses help against Smith-Njigba? Track participation and targets separately; leadership and good practice reports do not establish volume.
Separate injury-forced opportunity and score state from the healthy-roster baseline.

### NFL statistical audit — September 9, 2026

Available regular-season count fields were cross-checked against the [official club career table](https://www.seahawks.com/team/players-roster/cooper-kupp/).
The [audit snapshot](../../weekly/2026/week-01/research/ne-sea/audit/official-career-counts.csv)
retains the source counts; [verification limits](../../weekly/2026/week-01/research/ne-sea/audit/README.md)
separate targets, return statistics, college history and source-average discrepancies.
Blank source fields remain unverified; this audit does not independently confirm every
older target, return or postseason statistic. Derived averages use their stated
numerator/denominator. This check does not reverify
contracts, every scouting claim or current game status.

## Sources

- [Official club biography and career statistics](https://www.seahawks.com/team/players-roster/cooper-kupp/) — dynamic; NFL count columns checked 2026-09-09.

- [Seattle official biography and career statistics](https://www.seahawks.com/team/players-roster/cooper-kupp/) — dynamic; checked 2026-09-09.
- [John Boyle — Cooper Kupp heading into his tenth season](https://www.seahawks.com/news/seahawks-wr-cooper-kupp-continuing-to-show-he-s-a-force-multiplier-heading-into-10th-season) — published 2026-08-26; checked 2026-09-09.
