---
schema_version: 1
record_id: bw-2026-ten-registry-001
record_type: beat_writer_registry
title: "Tennessee Titans 2026 Source Registry"
team_ids: ["TEN"]
player_ids: []
season: 2026
week: null
status: active
time_horizon: seasonal
valid_as_of: 2026-08-21
last_verified: 2026-08-21
confidence: high
source_ids: []
supersedes: []
writer_ids: ["local-writer-turron-davenport", "local-writer-nick-suss", "local-writer-paul-kuharsky", "local-writer-easton-freeze", "local-writer-jim-wyatt"]
---

# Tennessee Titans 2026 Source Registry

See [sources.csv](sources.csv), [endpoints.csv](endpoints.csv), and [README.md](README.md).

## Essential monitoring

| Source | Outlet | Class | Primary value | Handling note |
|---|---|---|---|---|
| Tennessee Titans communications | Titans | official | Roster, injuries, transactions, depth chart and conferences | Team analysis is not independent |
| NFL gamebooks and participation | NFL | official | Participation and game records | Preferred postgame record |
| Turron Davenport | ESPN NFL Nation | reporting | Daily beat, practice, injuries and roster | ESPN verifies assignment |
| Nick Suss | The Tennessean | reporting | Daily beat, camp reps, personnel and organization | Paywalled; summarize and attribute |
| Paul Kuharsky | PaulKuharsky.com | reporting | Long-tenured independent beat, organization and practice | Separate reporting from commentary |

## Valuable monitoring

| Source | Outlet | Class | Primary value | Handling note |
|---|---|---|---|---|
| Easton Freeze | A to Z Sports | reporting | Practice, roster, scheme and local sourcing | Separate reporting from show opinion |
| Jim Wyatt | Titans | team analysis | Detailed camp observation, interviews and official context | Team employee; not independent confirmation |
| Pro Football Reference | Sports Reference | data | Logs and snap counts | Secondary reference |

## Reliability history

Established 2026-08-21. No repo-local outcome history has yet been scored.

### Independent beat rotation

- Strongest coverage areas: practice, quarterback development, skill-position roles, injuries,
  roster construction and organization.
- Known limitations: camp reporting restrictions, paywalls and mixed reporting/commentary formats.
- Confirmed early reports: none scored yet.
- Corrections or misses: none scored yet.
- Current reliability assessment: unscored; tiers reflect verified roles, tenure and camp access.
- Evidence for assessment: ESPN confirms Davenport's assignment; current 2026 Tennessean coverage
  confirms Suss; Kuharsky and Freeze publish current credentialed camp work.

## Usage notes

Use official reports for transactions and availability. Because Tennessee restricts some practice
details, corroborate camp conclusions and confirm them with preseason or regular-season usage.
