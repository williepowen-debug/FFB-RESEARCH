---
schema_version: 1
record_id: bw-2026-ind-registry-001
record_type: beat_writer_registry
title: "Indianapolis Colts 2026 Source Registry"
team_ids: ["IND"]
player_ids: []
season: 2026
week: null
status: active
time_horizon: seasonal
valid_as_of: 2026-08-21
last_verified: 2026-08-21
confidence: high
source_ids: []
supersedes: []
writer_ids: ["local-writer-stephen-holder", "local-writer-joel-a-erickson", "local-writer-nathan-brown", "local-writer-james-boyd", "local-writer-kevin-bowen"]
---

# Indianapolis Colts 2026 Source Registry

See [sources.csv](sources.csv), [endpoints.csv](endpoints.csv), and [README.md](README.md).

## Essential monitoring

| Source | Outlet | Class | Primary value | Handling note |
|---|---|---|---|---|
| Indianapolis Colts communications | Colts | official | Roster, injuries, transactions, depth chart and conferences | Team analysis is not independent |
| NFL gamebooks and participation | NFL | official | Participation and game records | Preferred postgame record |
| Stephen Holder | ESPN NFL Nation | reporting | Daily beat, injuries, organization and roster | ESPN verifies assignment |
| Joel A. Erickson | Indianapolis Star | reporting | Practice, scheme, personnel and roster | Separate reporting from analysis |
| Nathan Brown | Indianapolis Star | reporting | Practice, injuries, quarterbacks and organization | Separate reporting from analysis |
| James Boyd | The Athletic | reporting | Camp, injuries, roster construction and organization | Separate reporting from radio commentary |

## Valuable monitoring

| Source | Outlet | Class | Primary value | Handling note |
|---|---|---|---|---|
| Kevin Bowen | 93.5/107.5 The Fan | reporting | Camp observation, roster and local context | Separate reporting from radio opinion |
| Pro Football Reference | Sports Reference | data | Logs and snap counts | Secondary reference |

## Reliability history

Established 2026-08-21. No repo-local outcome history has yet been scored.

### Independent beat rotation

- Strongest coverage areas: availability, practice reps, quarterback health, role competition,
  scheme and organizational decisions.
- Known limitations: subscription access and formats that mix reporting with radio analysis.
- Confirmed early reports: none scored yet.
- Corrections or misses: none scored yet.
- Current reliability assessment: unscored; tiers reflect verified roles and recent camp access.
- Evidence for assessment: ESPN confirms Holder's assignment; current 2026 IndyStar camp coverage
  identifies Erickson and Brown as Colts insiders; current Athletic work confirms Boyd's role;
  and Bowen is actively reporting from camp.

## Usage notes

Use official reports for transactions and availability. Treat camp depth and role conclusions as
provisional until independently corroborated or confirmed by measured game usage.
