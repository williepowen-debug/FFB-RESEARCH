# Miami Dolphins source-monitoring guide

This directory tracks the people, outlets, official records, analysis, and data sources used to monitor the 2026 Miami Dolphins. It is designed for performance and fantasy-football research: injuries, availability, practice participation, depth-chart movement, player usage, coaching and scheme, transactions, and contract context.
...
- `endpoints.csv` contains websites, social accounts, podcast pages, feeds, and other monitoring endpoints.
...
An analyst and a reporter can both be essential without being interchangeable. An official injury report establishes a designation; a reporter may add practice context; a film analyst may explain the likely football effect.
...
1. Check official transactions and injury information.
2. Record absences, participation, first-team repetitions, position experiments, and return-to-play progression from credentialed observations.
3. Compare at least two independent reporters when a claim could materially change a player projection.
4. Use film and scheme analysts to interpret the football meaning only after the observed facts are captured.
5. Downgrade unsupported praise such as “best shape,” “breakout,” or “unstoppable” unless it is paired with role evidence.
...
3. Track snaps, routes, targets, carries, goal-line work, two-minute work, pass protection, and special-teams usage.
...
- Label team-employed analysis. It can contain strong practice and film detail, but it is not organizationally independent.
- Distinguish Omar Kelly's firsthand or sourced reporting from his evaluative commentary; apply the same rule to every hybrid reporter-columnist.
- Record role changes instead of silently deleting stale affiliations. Use `former_role`, `current_role`, `ended_on`, and `replacement_if_known` in `sources.csv`.
- Do not reproduce paywalled text, paid data exports, podcast transcripts, or substantial copyrighted material. Store links and original summaries.
- Reverify active roles and endpoints before camp, after major newsroom changes, and at least once during the regular season.

## Maintenance

Update `last_verified` whenever a role or endpoint is checked. Set a departed source to `inactive` rather than deleting the row. Add a new row only after confirming the person's current role from the outlet, program, or person's own current profile.
