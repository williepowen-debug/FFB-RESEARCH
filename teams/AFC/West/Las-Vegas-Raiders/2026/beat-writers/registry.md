---
schema_version: 1
record_id: bw-2026-lv-registry-001
record_type: beat_writer_registry
title: "Las Vegas Raiders 2026 Source Registry"
team_ids: ["LV"]
player_ids: []
season: 2026
week: null
status: active
time_horizon: seasonal
valid_as_of: 2026-08-21
last_verified: 2026-08-21
confidence: high
source_ids: []
supersedes: []
writer_ids: ["local-writer-ryan-mcfadden", "local-writer-adam-hill", "local-writer-hondo-carpenter", "local-writer-vincent-bonsignore", "local-writer-ed-graney", "local-writer-marcus-johnson", "local-writer-paul-gutierrez", "local-writer-levi-edwards"]
---

# Las Vegas Raiders 2026 Source Registry

See [sources.csv](sources.csv), [endpoints.csv](endpoints.csv), and [README.md](README.md).

## Essential monitoring

| Source | Outlet | Class | Primary value | Handling note |
|---|---|---|---|---|
| Las Vegas Raiders communications | Raiders | official | Roster, injuries, transactions, depth chart and conferences | Team analysis is not independent |
| NFL gamebooks and participation | NFL | official | Participation and game records | Preferred postgame record |
| Ryan McFadden | ESPN NFL Nation | reporting | Daily beat, camp, injuries and roster | ESPN verifies 2026 assignment |
| Adam Hill | Las Vegas Review-Journal | reporting | Local beat, camp, roster and breaking news | Separate reporting from podcast analysis |
| Hondo Carpenter | Raiders On SI | reporting | Credentialed camp work, roster and organization | Corroborate insider and projection claims |

## Valuable monitoring

| Source | Outlet | Class | Primary value | Handling note |
|---|---|---|---|---|
| Vincent Bonsignore | California Post / Raider Nation Radio | reporting | Organization sourcing and roster context | No longer primary beat; separate reporting from radio opinion |
| Ed Graney | Las Vegas Review-Journal | reporting | Camp columns, organization and local context | Separate sourced reporting from opinion |
| Marcus Johnson | Tape Don't Lie | film analysis | Scheme, line play, coverage and role review | Analysis source; establish facts independently |
| Paul Gutierrez | Las Vegas Raiders | team analysis | Column analysis, owner access and organizational context | Team contributor; not independent confirmation |
| Levi Edwards | Las Vegas Raiders | team analysis | Camp notebooks, interviews and official context | Team employee; not independent confirmation |
| Pro Football Reference | Sports Reference | data | Logs and snap counts | Secondary reference |

## Reliability history

Established 2026-08-21. No repo-local outcome history has yet been scored.

### Independent beat rotation

- Strongest coverage areas: quarterback development, camp reps, skill roles, injuries, roster and
  organization.
- Known limitations: the independent local beat contracted materially after 2025; several familiar
  voices changed roles or left the beat.
- Confirmed early reports: none scored yet.
- Corrections or misses: none scored yet.
- Current reliability assessment: unscored; use cross-source corroboration aggressively.
- Evidence for assessment: ESPN confirms McFadden's Raiders assignment; current Review-Journal
  coverage confirms Hill and Graney; Carpenter publishes dated credentialed 2026 camp reports;
  and current work confirms Bonsignore's columnist role and Johnson's film-analysis role.

## Usage notes

Use official reports for transactions and availability. Do not treat former beat assignments as
current: Tashan Reed and Vic Tafur left the Raiders beat, and Bonsignore is now a columnist.
